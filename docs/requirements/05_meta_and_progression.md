...
### **💾 로컬 저장 및 모바일 출시 데이터 계약 (Local Save Contract)**
Phase 9의 모바일 출시 준비에서는 서버 없이도 진행도와 설정을 안전하게 유지해야 합니다.

| 데이터 | 저장 필요 시점 | 비고 |
| :--- | :--- | :--- |
| Gem 잔고 및 영구 강화 | 구매/정산 직후 | 중복 구매 방지 |
| 업적 진행도 및 보상 수령 | 업적 이벤트/보상 수령 직후 | 보상은 1회만 지급 |
| 선택 터렛 및 설정 | 로비 변경 직후 | 볼륨, 언어, 입력 감도 포함 |
| 튜토리얼 완료 플래그 | 스킵/완료 직후 | 첫 실행 흐름 제어 |
| 세이브 스키마 버전 | 모든 저장 파일 | Phase 10 클라우드 세이브 마이그레이션 대비 |

* 저장은 원자적 쓰기(temp -> validate -> replace)를 사용합니다.
* 체크섬/검증값은 변조 탐지용이며, 강한 암호화로 간주하지 않습니다.
* 서버 동기화와 충돌 해결은 Phase 10 범위로 분리합니다.
