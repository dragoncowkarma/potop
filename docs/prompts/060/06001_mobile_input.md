# [Milestone 21] [Jules: Gemini 3.1 Pro] Mobile Touch & Joystick Control
...
- 현재 모듈: Assets/Scripts/Input/
- 관련 배경: 모바일 터치 및 가상 조이스틱 대응
- 연관 시스템: UNITY_ANDROID/IOS 전처리기
- **의존성**: Jules의 보스 AI 구현(06021_jules_boss_ai) 및 PR 머지 완료 후 시작
</context>
...
<task>
1. `../../SUMMARY.xml` 확인.
2. [Task: Mobile Input Implementation]
   - `MobileInputManager.cs`: 터치 델타 또는 가상 조이스틱 입력 매핑.
   - 플랫폼 전처리기를 이용한 코드 최적화 및 타겟 플랫폼 대응.
