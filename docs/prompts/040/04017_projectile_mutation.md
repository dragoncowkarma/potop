# [Milestone 17] [Jules: Gemini 3.1 Pro] Projectile Mutation Logic
...
- 현재 모듈: Assets/Scripts/Gameplay/Projectiles/
- 관련 배경: 투사체 변이(관통, 도탄, 폭발) 구현
- 연관 시스템: Physics Engine, PoolManager
</context>
...
<task>
1. `../../SUMMARY.xml` 확인.
2. [Task: Projectile Mutation Implementation]
   - `PiercingProjectile.cs`: RaycastAll 또는 충돌 카운터 기반 관통 로직.
   - `BouncingProjectile.cs`: Vector3.Reflect 기반 도탄 로직.
   - `ExplosiveProjectile.cs`: OverlapSphere 기반 범위 데미지 로직.
...
# 💻 Input
<input_data>
- `potop_client/Assets/Scripts/Combat/Projectile.cs`: 기존 투사체 베이스 클래스
- `../../gdd/03_data_and_balance.md`: 투사체 변이 관련 밸런스 데이터
- `../../gdd/07_development_milestones.md`: Milestone 17 상세 스펙
</input_data>
