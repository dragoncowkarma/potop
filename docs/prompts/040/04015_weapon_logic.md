# [Milestone 15] [Jules: Gemini 3.1 Pro] Modular Weapon Logic
...
- 관련 배경: Claude의 설계를 바탕으로 무기 시스템 모듈화
- 연관 시스템: TurretShooter, IWeapon
- **의존성**: Jules의 무기 아키텍처 설계(04015_jules_weapon_architecture) 및 PR 머지 완료 후 시작
</context>
...
2. [Task: Modular Weapon Logic Implementation]
   - Implement `WeaponBase.cs` and `TurretWeapon.cs` (Inheriting from `WeaponBase`).
   - Refactor `TurretShooter.cs`: `IWeapon` 인터페이스를 사용하도록 리팩토링.
...
# 💻 Input
<input_data>
- `potop_client/Assets/Scripts/Combat/TurretShooter.cs`: 기존 사격 로직 (리팩토링 대상)
- `docs/prompts/04015_P01_Claude_WeaponArchitecture.md`: 설계된 인터페이스 및 클래스 구조
</input_data>
