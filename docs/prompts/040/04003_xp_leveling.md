# [Milestone 16] [Jules: Gemini 3.1 Pro] Player XP & Leveling System
...
- 현재 모듈: Assets/Scripts/Gameplay/Progression/
- 관련 배경: XP 획득 및 레벨업 시스템 구축
- 연관 시스템: Event Broker, Upgrade UI
</context>
...
<task>
1. `../../SUMMARY.xml` 확인.
2. [Task: Player XP & Leveling Implementation]
   - Implement `LevelManager.cs`: `int _currentXP`, `int _currentLevel`, `AnimationCurve _xpRequirementCurve`.
   - Logic: Subscribe to `EnemyKilledEvent`. On level up, publish `LevelUpEvent`.
   - UI Interaction: `LevelUpEvent` 발생 시 업그레이드 메뉴 호출 트리거.
...
# 💻 Input
<input_data>
- `../../gdd/05_meta_and_progression.md`: 레벨업 공식 및 XP 테이블 데이터
- `potop_client/Assets/Scripts/Core/EventBroker.cs`: `EnemyKilledEvent` 정의 확인
- `../../gdd/07_development_milestones.md`: Milestone 16 상세 스펙
</input_data>
