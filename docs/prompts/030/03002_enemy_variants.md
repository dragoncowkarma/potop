# [Milestone 13] [Jules: Gemini 3.1 Pro] Specialized Enemy Variants
...
- 현재 모듈: Assets/Scripts/Gameplay/AI/
- 관련 배경: 적 변이(Blitz, Armored, Swarm) 구현
- 연관 시스템: PoolManager, EnemyBase
</context>
...
2. [Task: Specialized Enemy Variants Implementation]
   - Refactor `Enemy.cs` into `EnemyBase.cs`.
   - Implement `BlitzEnemy.cs` (High speed, low health, direct beeline).
   - Implement `ArmoredEnemy.cs` (High health, low speed, knockback ignore).
   - Implement `SwarmEnemy.cs` (Low health, spawns in clusters).
...
<input_data>
- `potop_client/Assets/Scripts/Combat/Enemy.cs`: 기존 적 베이스 클래스 및 로직
- `../../gdd/03_data_and_balance.md`: 적 변이 관련 밸런스 데이터 및 요구사항
- `../../gdd/07_development_milestones.md`: Milestone 13 상세 스펙
</input_data>
