...
- Current Module: Overclock Mode & Game Flow
- Background: 보스 처치 후 진입하는 무한 웨이브 스케일링(오버클럭) 및 전체 게임 루프 완성.
- Related Systems: WaveManager, GameManager, EventBroker

## Input Scope
- Strict Scope: 
  - `Assets/Scripts/Gameplay/Flow/OverclockMode.cs`
  - `Assets/Scripts/Gameplay/Flow/GameFlowController.cs`
  - `Assets/Data/Flow/OverclockData.asset`
- Reference (Read-only): `GameManager.cs`, `WaveManager.cs`, `EventBroker.cs`, `ResultScreenController.cs`
...
> **MANDATORY**: You MUST write to `docs/cycle_logs/7.3_log.md` BEFORE writing any code.
...
### Implementation Task
1. `OverclockMode.cs` 구현: 
   - `OnBossDefeated` 구독 시 오버클럭 모드 진입.
   - WaveManager를 무한 스폰 모드로 전환.
   - `Time.timeScale` 조작 없이, 30초마다 적 HP/속도/대미지 ×1.5 및 점수 배율 +0.1x 가산 (SO 데이터 활용).
2. `GameFlowController.cs` 구현: 
   - 전체 루프 관리: 로비 → 터렛 선택 → 인게임(15분) → 보스전 → 보스 처치 → 오버클럭 → 사망 → 결산.
   - 15분 타이머 제어 및 `OnTimerFinished` 이벤트 발생.
3. 결산 처리 연동: 
   - 처치 수, 최대 웨이브, 획득 Gem, 생존 시간 데이터를 취합하여 결산 화면 UI Controller로 전달.

### POTOP Constraints
- **[CRITICAL: STRICT SCOPE] 지정된 파일(Scope) 이외의 어떠한 파일도 임의로 수정, 포맷팅, 삭제하지 마십시오.**
- [Required] `GameFlowController`는 상태와 플로우 통제만 담당하며, `GameManager` 고유 기능과 역할 분리를 명확히 하십시오.
- [Required] 오버클럭 스케일링 로직은 프레임레이트 독립성을 유지(`Time.deltaTime` 기반)하십시오.
- [Prohibited] 멀티 씬 전환 로직을 금지하며, 단일 씬 내에서 상태 전환 기반으로 플로우를 제어하십시오.
...
| Task Type | Command |
|---|---|
| `*-RED` tasks | `[PATH]/harness.sh test --mode tdd-red --id 7.3 --cmd "{cmd}"` |
| `*-GREEN` tasks | `[PATH]/harness.sh test --id 7.3 --cmd "{cmd}"` |
...
```bash
[ABSOLUTE_SKILL_PATH]/scripts/harness.sh test --id 7.3 --cmd "{cmd}" --mutation
```
...
```bash
[ABSOLUTE_SKILL_PATH]/scripts/harness.sh test --id 7.3 --cmd "{validation_command}"
```
...
- **On Failure**: Update `docs/tasks/7.3.json` with status `[Failed]` and analyze `coverage/lcov.info` to find untested paths.
...
1. **<step 1>** Use your file-writing tool to create/update `docs/cycle_logs/7.3_log.md` with your Intent, Analysis, Plan, and Failure Modes.
2. **<step 2>** Use your file-editing tool to implement the required code changes in the target files.
3. **<step 3>** Use your shell execution tool to run the validation command: `[PATH]/harness.sh test --id 7.3 --cmd "..."`
