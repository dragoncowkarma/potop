# [Milestone 18] [Jules: Gemini 3.1 Pro] Combat Juice & Feedback
...
- 현재 모듈: Assets/Scripts/VFX/
- 관련 배경: 카메라 쉐이크 및 피격 이펙트 연동
- 연관 시스템: Cinemachine, Event Broker
</context>

# 🛠️ Task
다음 지시사항을 `../../AGENTS.md` 프로세스에 따라 수행하십시오.
<task>
1. `../../SUMMARY.xml` 읽기 및 `../../../REFACTOR_TRACKING.md` 확인.
2. [Task: Combat Juice Implementation]
   - `CameraShakeController.cs`: `CinemachineImpulseSource`를 이용한 쉐이크 구현.
   - `HitFlash.cs`: Material Property Block을 이용한 피격 플래시 구현.
   - 연동: 탄환 발사 및 적 사망 이벤트에 쉐이크와 플래시 연결.
...
# 💻 Input
<input_data>
- `potop_client/Assets/Scripts/Combat/Enemy.cs`: 피격 로직 연동 대상
- `potop_client/Assets/Scripts/Combat/TurretShooter.cs`: 발사 로직 연동 대상
- `../../gdd/06_art_and_sound.md`: 시각 효과 및 타격감 가이드라인
</input_data>
