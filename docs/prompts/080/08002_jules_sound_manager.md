...
> **MANDATORY**: You MUST write to `docs/cycle_logs/8.2_log.md` BEFORE writing any code.
...
| Task Type | Command |
|---|---|
| `*-RED` tasks | `[PATH]/harness.sh test --mode tdd-red --id 8.2 --cmd "{cmd}"` |
| `*-GREEN` tasks | `[PATH]/harness.sh test --id 8.2 --cmd "{cmd}"` |
...
```bash
[ABSOLUTE_SKILL_PATH]/scripts/harness.sh test --id 8.2 --cmd "{cmd}" --mutation
```
...
```bash
[ABSOLUTE_SKILL_PATH]/scripts/harness.sh test --id 8.2 --cmd "{validation_command}"
```
...
- **On Failure**: Update `docs/tasks/8.2.json` with status `[Failed]` and analyze `coverage/lcov.info` to find untested paths.
...
1. **<step 1>** Use your file-writing tool to create/update `docs/cycle_logs/8.2_log.md` with your Intent, Analysis, Plan, and Failure Modes.
2. **<step 2>** Use your file-editing tool to implement the required code changes in the target files.
3. **<step 3>** Use your shell execution tool to run the validation command: `[PATH]/harness.sh test --id 8.2 --cmd "..."`
