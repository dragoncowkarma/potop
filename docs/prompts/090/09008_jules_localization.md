...
# [TARGET: Assets/Scripts/Core/Localization/LocalizationManager.cs] [TASK: 9.8]
...
> **MANDATORY**: You MUST write to `docs/cycle_logs/9.8_log.md` BEFORE writing any code.
...
**Target File**: `Assets/Scripts/Core/Localization/LocalizationManager.cs`
...
| Task Type | Command |
|---|---|
| `*-RED` tasks | `[PATH]/harness.sh test --mode tdd-red --id 9.8 --cmd "{cmd}"` |
| `*-GREEN` tasks | `[PATH]/harness.sh test --id 9.8 --cmd "{cmd}"` |
...
```bash
[ABSOLUTE_SKILL_PATH]/scripts/harness.sh test --id 9.8 --cmd "{cmd}" --mutation
```
...
```bash
[ABSOLUTE_SKILL_PATH]/scripts/harness.sh test --id 9.8 --cmd "{validation_command}"
```
...
- **On Failure**: Update `docs/tasks/9.8.json` with status `[Failed]` and analyze `coverage/lcov.info` to find untested paths.
...
1. **<step 1>** Use your file-writing tool to create/update `docs/cycle_logs/9.8_log.md` with your Intent, Analysis, Plan, and Failure Modes.
2. **<step 2>** Use your file-editing tool to implement the required code changes in the target files.
3. **<step 3>** Use your shell execution tool to run the validation command: `[PATH]/harness.sh test --id 9.8 --cmd "..."`
