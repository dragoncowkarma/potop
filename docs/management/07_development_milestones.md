...
> [!IMPORTANT]
> **Phase 8 목표는 기능 추가가 아니라 측정 가능한 게임필 품질 확보입니다.** 히트스탑, 오디오, VFX, 밸런스는 GC/TimeScale/풀링/시뮬레이션 증거로 검증합니다.
> [!TIP]
> AI Prompts:
...
*   **[08001] 컴뱃 쥬스 최종 정비** — 중앙 TimeController, 히트스탑/슬로우모션 중첩 복구, 프레임 독립 카메라 흔들림, 위협 인디케이터 연결
*   **[08002] 사운드 시스템** — Centralized SoundManager + SFX 풀링 + Audio Mixer 그룹 + 동시 재생 수 제한
*   **[08003] VFX 최종 정비** — 적 사망/보스 페이즈/오버클럭 연출, Particle 완전 리셋, 모바일 품질 폴백
...
*   **[09001] 로비 UI 완성** — 터렛 선택/강화 상점/설정 화면 + Safe Area/모바일 비율 대응 + 저장 상태 핸드오프
*   **[09002] 업적 시스템** — AC_001~AC_010, 보상 1회 지급, 저장 가능한 스냅샷 계약
*   **[09003] 로컬 세이브 시스템** — 버전 있는 JSON, 원자적 쓰기, 백업 복구, 변조 탐지, 마이그레이션
*   **[09004] 모바일 입력 최적화** — 조이스틱/자이로/자동 사격 + 180°/sec 회전 상한 + 키보드 폴백
*   **[09005] 모바일 광고 연동** — 보상형 부활(1회 한정), 전면 광고(3~5판 주기), no-ads/동의/실패 경로
*   **[09006] 모바일 최적화** — 성능 프로파일링, 배터리/발열, LOD, 품질 티어, Draw Call 150 이하 목표
*   **[09007] 튜토리얼 시스템** — Phase 7 프로토타입 하드닝, 스킵/재시작/저장 플래그, 모바일 입력 연계
*   **[09008] 다국어 지원** — KO/EN/JP 최소 3개 언어, 하드코딩 문자열 감사, 런타임 언어 전환
*   **[09009] 스토어 등록 & 심사** — AOT/IL2CPP 검증, 릴리즈 로그 제거, 광고/개인정보/스크린샷 체크리스트
*   **[09099] 모바일 검증** — First Launch -> Tutorial -> Lobby -> In-Game -> Revive/Settlement -> Lobby 전체 루프 검증

---

### **Phase 10: 글로벌 서버 (Server Infrastructure)** `[예상: 4~6주]`
> [!NOTE]
> Phase 10 착수 전 Phase 9.99 산출물에서 로컬 세이브 스키마, 업적 스냅샷, 점수 이벤트 로그, 광고/동의 상태 계약이 확정되어야 합니다.
